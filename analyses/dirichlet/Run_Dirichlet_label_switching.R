...
source("../../donors/PD37113/dirichlet/Mutation_Cluster_Dirichlet_Gibbs_sampler_functions.R")
...
fwrite(ls_tbl, file.path(output_dir, 'clust_assign_gs.csv'))
...
post.cluster.pos <- post.param.dist(gs.out = lymph.gs, ls.merge.out = lymph.merge.ls, 
...
mut.spec.by.clust <- context.extract(mut.spec, lymph.merge.ls)
...
for (i in which.clusters) {
  temp.plot <- cluster.density.plot(lymph.gs, lymph.merge.ls, post.cluster.pos, i, samp.names = short.names)
  print(temp.plot)
}
